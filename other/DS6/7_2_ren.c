...
{                                                                 //按照记录条数初始化哈希表
    HashList *hash = (HashList *)malloc(sizeof(HashList));        //分配哈希表空间
    hash->size = nextprime(n);                                    //获取哈希表长度
    hash->linklist = (Node *)malloc((hash->size) * sizeof(Node)); //结点分配空间
...
{                                               //基于身份证号求是否已存在该人,若存在，返回其地址
    int index = mhash(hash, id) % (hash->size); //先通过哈希函数求出哈希值，得到下标
...
{                             //若该人不存在，新记录插入到哈希表中，若已存在，更新长度记录
    ...
    {                                                 //如果该人是第一次记录（哈希算出的下标的下一个结点的地址为空时）
        ...
        int index = mhash(hash, id) % (hash->size);   //通过其身份证号求出相应哈希值
