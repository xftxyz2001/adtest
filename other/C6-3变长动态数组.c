// 6-3 变长动态数组 (30 分)
// 在C/C++中，原生数组的空间大小是静态的。例如，声明int a[10]之后，数组a的大小就已经固定为10，
// 因此在使用时，常常以数据量的上限作为数组的长度，比如最多有可能要保存100,000个元素时，
// 可以创建一个长度为10^6的数组。这样做的显著缺点就是，当实际数据量较小时，会造成大量的空间浪费。
// 更合理的方法是因地制宜地对数组空间进行动态分配。这里给出一种最简单的动态数组管理策略：

// 使用用户分配的一段连续的内存空间表示数组；
// 初始时，数组的容量为16；
// (扩展) 当插入元素时，如果空间已满，将数组容量增加至原有的2倍，也就是将原有的空间释放，
// 分配一个大小为原有大小2倍的内存空间，并将原来的元素复制到新的空间中，例如原来的空间大小为16，现在增加到32；
// (压缩) 删除数组中元素后，如果剩余元素的数量小于容量的1/4（向下取整）且数组的容量大于16时，
// 删除掉原有的空间，分配一个为原有容量一半的新的内存空间，并将原来的元素复制到新的空间中，例如原来的容量为64，
// 但是删除元素后，数组中只有15个元素，那么将容量压缩到32。
